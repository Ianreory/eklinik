...
@section('judul')
    <h1>Pasien Persalinan {{ $pasien->nama }}</h1>
@endsection
@section('judul2')
            ...
            @foreach ($persalinan as $persalinan)
                <tbody>
                    <tr>
                        ...
                        <td>{{ $persalinan->nama_ibu }}</td>
                        <td>{{ $persalinan->alamat }}</td>
                        <td>{{ $persalinan->sumber_pembiayaan }}</td>
                        <td class="center-button">
                            <div class="btndetail">
                                <form action="{{ route('surat_persalinan', $persalinan->id) }}" method="get">
                                ...
                                <form action="{{ route('delete_persalinan', $persalinan) }}" method="post">
                                    @method('delete')
                                    @csrf
                                    <button type="submit" class="btnumum ">Delete</button>
                                </form>
                                <form action="{{ route('edit_persalinan', $persalinan) }}" method="get">
