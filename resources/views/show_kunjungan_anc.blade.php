            ...
            @foreach ($anc as $anc)
                <tbody>
                    <tr>
                        ...
                        <td> {{ $anc->pasien ? $anc->pasien->nama : 'Pasien Not Found' }} </td>
                        <td>{{ $anc->alamat }}</td>
                        <td class="center-button">
                            <div class="btndetail">
                                <form action="{{ route('show_catatan_anc', $anc) }}" method="get">
                                ...
                                <form action="{{ route('edit_anc', $anc) }}" method="get">
                                    <button class="btnumum1" type="submit">Update</button>
                                </form>
                                <form action="{{ route('delete_anc', $anc) }}" method="post">
                                    @method('delete')
                                    @csrf
