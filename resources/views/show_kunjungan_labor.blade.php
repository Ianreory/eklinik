...
@section('judul')
    <h1>Pasien Labor {{ $pasien->nama }}</h1>
@endsection
@section('judul2')
    Kunjungan Labor
@endsection
            ...
            <tbody>
                @foreach ($labor as $labor)
                    <tr>
                        ...
                        <td>{{ $labor->tanggal }}</td>
                        <td>{{ $labor->nama }}</td>
                        <td>{{ $labor->keterangan }}</td>
                        <td class="center-button">
                            <div class="btndetail">
                                <form action="{{ route('surat_labor', $labor->id) }}" method="get">
                                ...
                                <form action="{{ route('edit_labor', $labor) }}" method="get">
                                    <button class="btnumum1" type="submit">Update</button>
                                </form>
                                <form action="{{ route('delete_labor', $labor) }}" method="post">
                                    @method('delete')
                                    @csrf
