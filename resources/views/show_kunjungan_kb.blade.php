            ...
            @foreach ($kb as $kb)
                <tbody>
                    <tr>
                        ...
                        <td>{{ $kb->tanggal }}</td>
                        <td>{{ $kb->nama }}</td>
                        <td>{{ $kb->hasil_pemeriksaan }}</td>
                        <td class="center-button">
                            <div class="btndetail">
                                <form action="{{ route('surat_kb', $kb->id) }}" method="get">
                                ...
                                <form action="{{ route('edit_kb', $kb) }}" method="get">
                                    <button class="btnumum1" type="submit">Update</button>
                                </form>
                                <form action="{{ route('delete_kb', $kb) }}" method="post">
                                    @method('delete')
                                    @csrf
